...
public class Aerolinea
{
    ...
    public Collection<Tiquete> getTiquetes( )
    {
    	...
    	Collection<Tiquete> tiquetesAerolinea = new ArrayList<>();
    	
    	for(Vuelo v: vuelos) {
    		tiquetesAerolinea.addAll(v.getTiquetes());
    ...
    public void cargarAerolinea( String archivo, String tipoArchivo ) throws TipoInvalidoException, IOException, InformacionInconsistenteException
    {
    	...
    	IPersistenciaAerolinea cargar = CentralPersistencia.getPersistenciaAerolinea(tipoArchivo);
    	cargar.cargarAerolinea(archivo, this);
    }
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	if (ruta == null) throw new Exception("No existe esta ruta"); //Si la ruta no existe se lanza una excepcion
    	
    	Avion avionEncontrado = null;
    	for(Avion a: aviones) {
    		if(a.getNombre()==nombreAvion) {
    			avionEncontrado = a;
    			break; //Se usa para detener el ciclo una vez haya sido encontrado el avion
    		}
    	}

    	if (avionEncontrado == null) throw new Exception("No existe este avion"); //Si el avion no existe se lanza una excepcion
    	
    	for(Vuelo v: vuelos) {
    		if(v.getAvion() == avionEncontrado && v.getRuta().getCodigoRuta() == codigoRuta) throw new Exception("El avion ya tiene una ruta diferente programada en esa fecha");
    	}
    	
    	Vuelo vuelo = new Vuelo(ruta,fecha,avionEncontrado);
    	vuelos.add(vuelo);
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	...
    	if(!existeCliente(identificadorCliente)) throw new Exception("No existe este cliente");
    	Cliente cliente = getCliente(identificadorCliente);
    	
    	String[] partesFecha = fecha.split("-");
        int mes = Integer.parseInt(partesFecha[1]); //Se obtiene el mes para luego determinar temporada baja o alta
    	
        CalculadoraTarifas calcTB = new CalculadoraTarifasTemporadaBaja();//Calculadora temporada baja
        CalculadoraTarifas calcTA = new CalculadoraTarifasTemporadaBaja();//Calculadora temporada alta
        
        Vuelo vuelo = getVuelo(codigoRuta,fecha);
        
    	if((mes >= 1 && mes <= 5) || (mes >= 9 && mes <= 11)) {
    		costo = vuelo.venderTiquetes(cliente, calcTB, cantidad);
    	}
    	else if((mes >= 6 && mes <= 9) || (mes == 12)) {
    		costo = vuelo.venderTiquetes(cliente, calcTA, cantidad);
    	}
    			
    	if(costo == -1) throw new VueloSobrevendidoException(vuelo);
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
    	...
    	String total = String.valueOf(cliente.calcularValorTotalTiquetes());
